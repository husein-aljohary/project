...
public class Park_space extends AppCompatActivity {
    ...
    ArrayList<Parkingspace> lists;
    MyAdapter3 myAdapter3;
    private final DatabaseReference database= FirebaseDatabase.getInstance().getReference();
    private final List<Parkingspace> myspaceList=new ArrayList<>();
        ...
        database.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot mallinfo : snapshot.child("mallinfo").child(city).child(mallname).child("space").getChildren()){
                    //*************change the status*************
                    //database.child("mallinfo").child(city).child(mallname).child("space").child("0").child("status").setValue("false");
                    //*************change the status*************
                    ...
                    Parkingspace myspace=new Parkingspace(getNumber,getStatus);
                ...
                recyclerViewspace.setAdapter(new MyAdapter3(Park_space.this, (ArrayList<Parkingspace>) myspaceList){
